...
def crawling_detail(keyword):
    ...
    token_receive = request.cookies.get('mytoken')
    # 코멘트 불러오기
    comments_name = db.wine.find_one({'post_num': find_keyword}, {
                                     'COMMENT': 1, '_id': False})
...
def save_comment():
    ...
    if pageInfo_receive == "WineNOT":
        ...
        comments = db.Reviews.find_one({'post_num': postNum_receive}, {
                                       'COMMENT': 1, '_id': False})
        ...
        db.Reviews.update_many({'post_num': postNum_receive}, {
                               '$addToSet': {'COMMENT': doc}})

    elif pageInfo_receive == "Weinco":
        # DB에 코멘트의 마지막 ID 값 읽어서 +1
        comments = db.wine.find_one({'post_num': postNum_receive}, {
                                    'COMMENT': 1, '_id': False})
...
def winelist():
    ...
    if board_name == 'vivino':
        posts_list = list(db.vivino_wines.find(
            {}, {'_id': False}).sort('post_num', -1))
    elif board_name == 'weinco':
        posts_list = list(db.weinco_wines.find(
            {}, {'_id': False}).sort('post_num', -1))
    elif board_name == 'xtrawine':
        posts_list = list(db.xtra_wines_list.find(
            {}, {'_id': False}).sort('post_num', -1))
    elif board_name == 'wine21':
        posts_list = list(db.wine21.find({}).limit(
            {}, {'_id': False}).sort('post_num', -1))
